...
import { selectProps, selectEmits, useSelect } from '../../utils/select';
...
const handleOptionClick = (item:any) => {
  if (!item.disabled) {
    if (multiple.value) {
      const isRemove = modelValue.value.includes(item.value)  ? modelValue.value.splice(modelValue.value.indexOf(item.value), 1): modelValue.value.push(item.value);
